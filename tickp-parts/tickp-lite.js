        ...
        */
        _doplot : function(ctx, dataset, shift) {
            ...
            */
            ...
            /* try support resistances 
            var highs = dataset.ressupport.resistances;
            for(var i in highs) {
                if (highs[i] < ymin) 
                    continue;
                if(highs[i] > ymax)
                    continue;
                var loghigh = _log(highs[i]);
                var y1 = Math.round((loghigh - cp.ymin)*scale);
                if( y1 > 20) { // don't draw anything in first 20 pixels  
                    _drawline(ctx, _left, h-y1, _right, h-y1, "#FF0000", 1.0);
                    label = highs[i];
                    this._drawText(label, _right, h - y1, {align:'left', padding:5});
                } 
            };
            var lows = dataset.ressupport.supports;
            for(var i in lows) {
                if (lows[i] < ymin) 
                    continue;
                if(lows[i] > ymax)
                    continue;
                var loglow = _log(lows[i]);
                var y1 = Math.round((loglow - cp.ymin)*scale);
                if( y1 > 20) { // don't draw anything in first 20 pixels  
                    _drawline(ctx, _left, h-y1, _right, h-y1, "#FF0000", 1.0);
                    label = lows[i];
                    this._drawText(label, _right, h - y1, {align:'left', padding:5});
        ...
        getResSupport: function() {
            ...
            for (i = 1; i < ohlc.length; i++) {
                ...
                if((cur[1] >= last[1]) && (cur[2] >= last[2])) { // || (cur[3] >= last[3]))) { 
                ...
                if ((cur[2] < last[2]) && (cur[1] < last[1])) { //  || (cur[3] < last[3]))) {
                    trend = -1;
                    if(curtrend == -1) {  
                        suparray.pop();
                    } 
        ...
        accumulate : function(ts, data, volume, factor, name) { 
